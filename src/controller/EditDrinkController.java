 ...
 * @author DELL
 */
public class EditDrinkController implements Initializable {
    ...
    void clickSubmit(ActionEvent event) throws IOException, SQLException {
        if (!validateInfo()) {
            ...
            boolean result = MealDBHelper.updateDrink(txtName.getText(), Float.parseFloat(txtPrice.getText()), fileName,  DrinksManagerController.drinkCategoryNameId.get(cbCategory.getValue()), this.drink.getId() );
            if (result) {
                Navigator.getInstance().goToDrinkManager();
            } else {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        ObservableList<String> listDrinkStr = FXCollections.observableArrayList();
        for (DrinkCategory drinkCategory: listDrink
        ) {
            listDrinkStr.add(drinkCategory.getDrinkCategory());
        }
        cbCategory.setItems(listDrinkStr);
    }
    public void loadData(Meal drink){
        Image img = new Image(getClass().getResourceAsStream("/img/images/" + drink.getImage()));
        txtName.setText(drink.getName());
        cbCategory.setValue(drink.getCategory());
